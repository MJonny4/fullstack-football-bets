...
import type { AppTab, Bet, BettingLeaderboardEntry, LeagueStandings, LineupInput, PlaceBetInput, Round, Team } from './types';

const NAVIGATION: { id: AppTab; label: string; mobileLabel?: string; icon: IconName }[] = [
  { id: 'matches', label: 'Matches', icon: 'ball' },
  { id: 'standings', label: 'League', icon: 'table' },
  { id: 'bets', label: 'My bets', icon: 'ticket' },
  { id: 'leaderboard', label: 'Bettors', icon: 'ranking' },
  { id: 'team', label: 'My team', mobileLabel: 'Team', icon: 'shirt' },
];
...
function Dashboard() {
  ...
  const [tab, setTab] = useState<AppTab>('matches');
  ...
  const selectedNavigation = useMemo(() => NAVIGATION.find((item) => item.id === tab), [tab]);
    ...
    <div className="min-h-screen bg-cream pb-24 lg:pb-0">
          ...
          <nav className="hidden items-center gap-1.5 rounded-[1.35rem] border border-white/15 bg-black/15 p-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_12px_24px_-16px_rgba(0,0,0,0.9)] ring-1 ring-black/10 lg:flex" aria-label="Primary navigation">
            {NAVIGATION.map((item) => (
              <button
                ...
                className={`flex items-center gap-2 rounded-2xl px-3 py-3 text-sm font-bold transition-all duration-200 focus-visible:outline-white motion-reduce:transform-none xl:px-4 ${tab === item.id ? '-translate-y-0.5 bg-gradient-to-b from-pitch-200 to-pitch-300 text-pitch-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_4px_0_#0c754f,0_9px_18px_-9px_rgba(0,0,0,0.9)]' : 'text-white/65 hover:-translate-y-0.5 hover:bg-white/10 hover:text-white'}`}
                key={item.id}
              ...
              >
                ...
                {item.label}
                {item.id === 'bets' && bets.length > 0 && <span className={`rounded-full px-1.5 py-0.5 text-[9px] ${tab === item.id ? 'bg-pitch-950/10 text-pitch-950' : 'bg-white/10 text-white/70'}`}>{bets.length}</span>}
              </button>
            ))}
          </nav>
      ...
      <main className="mx-auto max-w-[1240px] px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
        <div className="mb-5 flex items-center gap-2 text-xs font-bold text-slate-400 lg:hidden">
          <Icon className="h-4 w-4" name={selectedNavigation?.icon ?? 'ball'} />
          <span>{selectedNavigation?.label}</span>
        </div>
        ...
        {loading ? (
          ...
          <>
            {tab === 'matches' && <MatchesPage balance={toNumber(user.coinBalance)} onPlaceBet={placeBet} round={round} />}
            {tab === 'standings' && <LeagueTablePage standings={standings} user={user} />}
            {tab === 'bets' && <BetsPage bets={bets} onBrowse={() => setTab('matches')} />}
            {tab === 'leaderboard' && <LeaderboardPage connected={socketConnected} entries={leaderboard} user={user} />}
            {tab === 'team' && <TeamPage onClaim={claimTeam} onSaveLineup={saveLineup} teams={teams} user={user} />}
          </>
        )}
      </main>
      ...
      <nav className="fixed inset-x-3 bottom-3 z-40 grid grid-cols-5 rounded-[1.4rem] border border-white/15 bg-gradient-to-b from-pitch-900/95 to-pitch-950/95 p-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.12),0_18px_38px_-12px_rgba(4,43,31,0.8)] ring-1 ring-black/10 backdrop-blur-xl lg:hidden" aria-label="Mobile navigation">
        {NAVIGATION.map((item) => (
          <button
            aria-current={tab === item.id ? 'page' : undefined}
            className={`flex flex-col items-center gap-1 rounded-2xl px-1 py-2.5 text-[10px] font-extrabold transition-all duration-200 focus-visible:outline-white motion-reduce:transform-none ${tab === item.id ? '-translate-y-1 bg-gradient-to-b from-pitch-200 to-pitch-300 text-pitch-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_4px_0_#0c754f,0_10px_18px_-10px_rgba(0,0,0,0.9)]' : 'text-white/55 hover:bg-white/10 hover:text-white'}`}
            key={item.id}
            onClick={() => setTab(item.id)}
            type="button"
          >
