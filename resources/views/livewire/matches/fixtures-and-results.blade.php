    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="flex flex-wrap items-center gap-4 mb-6">
                <div class="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
                    <button wire:click="setView('upcoming')"
                        class="px-4 py-2 text-sm font-medium rounded-md transition-colors {{ $selectedView === 'upcoming' ? 'bg-white dark:bg-gray-600 text-th-blue dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200' }}">
                        🔮 Upcoming ({{ $matchCounts['upcoming'] }})
                    </button>
                    <button wire:click="setView('results')"
                        class="px-4 py-2 text-sm font-medium rounded-md transition-colors {{ $selectedView === 'results' ? 'bg-white dark:bg-gray-600 text-th-blue dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200' }}">
                        📊 Results ({{ $matchCounts['finished'] }})
                    </button>
                    @if ($matchCounts['live'] > 0)
                        <button wire:click="setView('live')"
                            class="px-4 py-2 text-sm font-medium rounded-md transition-colors {{ $selectedView === 'live' ? 'bg-white dark:bg-gray-600 text-th-blue dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200' }}">
                            🔴 Live ({{ $matchCounts['live'] }})
                        </button>
                    @endif
                    <button wire:click="setView('all')"
                        class="px-4 py-2 text-sm font-medium rounded-md transition-colors {{ $selectedView === 'all' ? 'bg-white dark:bg-gray-600 text-th-blue dark:text-blue-400 shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200' }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model.live.debounce.300ms="searchTerm"
                        ...
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-th-blue focus:border-th-blue text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400">
                </div>
                ...
                <div>
                    ...
                    <select wire:model.live="selectedGameweek"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-th-blue focus:border-th-blue text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($matches as $match)
                <div
                    class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                    <!-- Match Header -->
                    <div class="bg-gradient-to-r from-gray-50 to-white dark:from-gray-700 dark:to-gray-800 p-4 border-b border-gray-200 dark:border-gray-600">
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="mx-6 text-center">
                                ...
                                @else
                                    <div class="text-3xl font-bold text-gray-400 dark:text-gray-500">VS</div>
                        ...
                        @else
                            @if ($selectedView !== 'results')
                                <div class="mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
                                    <a href="{{ route('login') }}"
                                        class="block w-full bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 py-2 px-4 rounded-lg font-medium text-center transition-colors">
                                        🔐 Login to Bet
                                    </a>
                                </div>
                            @endif
            ...
            @empty
                <div class="col-span-full">
                    <div class="text-center text-gray-500 dark:text-gray-400 py-16">
                        ...
                        <h3 class="text-xl font-medium text-gray-900 dark:text-gray-100 mb-2">No matches found</h3>
                        <p>Try adjusting your filters or check back later.</p>
                        @if ($selectedGameweek || $selectedTeam || $selectedStatus !== 'all' || $searchTerm)
                            <button wire:click="clearFilters"
                                class="mt-4 inline-flex items-center px-4 py-2 bg-th-blue hover:bg-th-red text-white rounded-lg transition-colors">
                                🗑️ Clear All Filters
                            </button>
                        @endif
                    </div>
