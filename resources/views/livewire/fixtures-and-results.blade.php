        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center">
                <h1 class="text-4xl font-bold mb-2 flex items-center justify-center">
                    <img src="{{ asset('images/goalguessers.png') }}" alt="GoalGuessers Logo" class="w-10 h-10 mr-3 object-contain">
                    Fixtures & Results
                </h1>
                            ...
                            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6 border-l-4 border-th-blue hover:shadow-2xl transition-all">
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex-1 text-center">
                                        <img
                                            src="{{ asset('images/teams/' . Str::slug($match->homeTeam->name) . '.png') }}"
                                    ...
                                    <div class="flex-1 text-center">
                                        <img
                                            src="{{ asset('images/teams/' . Str::slug($match->awayTeam->name) . '.png') }}"
                                ...
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <div class="font-bold text-lg">
                                        @if($match->home_goals !== null && $match->away_goals !== null)
                                            <span class="{{ $match->home_goals > $match->away_goals ? 'text-green-600' : ($match->home_goals < $match->away_goals ? 'text-red-600' : 'text-yellow-600') }}">
                                                {{ $match->home_goals }}
                                            </span>
                                            <span class="text-gray-400 mx-1">-</span>
                                            <span class="{{ $match->away_goals > $match->home_goals ? 'text-green-600' : ($match->away_goals < $match->home_goals ? 'text-red-600' : 'text-yellow-600') }}">
                                                {{ $match->away_goals }}
                                            </span>
                                        @else
            ...
            <div class="flex flex-wrap items-center gap-4 mb-6">
                ...
                @if($selectedGameweek || $selectedTeam || $selectedStatus !== 'all' || $searchTerm)
                    <button
                        ...
                        class="text-sm text-th-red hover:text-red-700 font-medium"
                    >
                        🗑️ Clear Filters
                    </button>
                @endif
            ...
            @forelse($matches as $match)
                <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden">
                    ...
                    <div class="bg-gradient-to-r from-gray-50 to-white p-4 border-b">
                        ...
                        <div class="mt-1">
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $match->getStatusBadgeClass() }}">
                        ...
                        @auth
                            @if($match->isBettingAvailable() && $selectedView !== 'results')
                                <div class="mt-6 pt-4 border-t border-gray-100">
                                    <button class="w-full bg-gradient-to-r from-th-blue to-th-red text-white py-2 px-4 rounded-lg font-medium hover:from-th-red hover:to-th-blue transition-all">
                                        🎯 Place Bet
                                    </button>
                                </div>
                            @endif
